...
namespace Win7FTP
{
    public partial class frmDownload : GlassForm
    {
        ...
        void ShowCompleteDownloadDialog()
        {
            ...
            taskDialogMain.Caption = "文件下载完成";
            taskDialogMain.InstructionText = "选择使用下载文件执行的操作:";
            taskDialogMain.FooterText = FileName + " 已经成功下载. 请选择要执行的操作或单击“取消和返回”.";
            ...
            TaskDialogCommandLink btnOpenFile = new TaskDialogCommandLink("打开文件", "打开下载文件");
            btnOpenFile.Click += new EventHandler(btnOpenFile_Click);

            TaskDialogCommandLink btnOpenFolder = new TaskDialogCommandLink("打开文件夹", "打开并继续下载文件");
        ...
        void FtpClient_OnDownloadProgressChanged(object sender, DownloadProgressChangedArgs e)
        {
            ...
            this.Text = PercentProgress.ToString() + "% 下载中 " + FileName;
            lblDownloadStatus.Text = "下载状态: 已下载 " + GetFileSize(e.BytesDownloaded) + " 还剩 " + GetFileSize(e.TotleBytes) + " (" + PercentProgress.ToString() + "%)";
        }
        #endregion
