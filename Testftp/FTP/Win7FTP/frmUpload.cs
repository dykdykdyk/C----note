...
namespace Win7FTP
{
    public partial class frmUpload : GlassForm
    {
        ...
        void FtpClient_OnUploadProgressChanged(object sender, UploadProgressChangedArgs e)
        {
            ...
            this.Text = PercentProgress.ToString() + " % 上传中 " + FileName;
            lblDownloadStatus.Text = "上传状态: 已上传 " + GetFileSize(e.BytesUploaded) + " 还剩 " + GetFileSize(e.TotleBytes) + " (" + PercentProgress.ToString() + "%)";
        ...
        void FtpClient_OnUploadCompleted(object sender, UploadCompletedArgs e)
        {
            if (e.UploadCompleted)
            {
                //No Error
                this.Text = "上传完成!";
                lblDownloadStatus.Text = "上传文件成功!";
                //change value
                progressBar1.Value = progressBar1.Maximum;
                btnCancel.Text = "退出";
                MessageBox.Show("上传文件成功!", "上传文件消息", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                //Change Texts of various controls.
                lblDownloadStatus.Text = "上传状态: " + e.UploadStatus;
                this.Text = "上传错误!";
                btnCancel.Text = "退出";
                ...
                MessageBox.Show("错误: " + e.UploadStatus, "上传错误!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
