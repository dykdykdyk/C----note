...
namespace FileExplorer
{
    ...
    private void NodeUpdate(TreeNode node)
        ...
        private void ListUpdate2(string newPath)
        {
            if (newPath == "")
                ListDrivers2();
            else
            {
                try
                {
                    ...
                    foreach (DirectoryInfo dir in dirs)
                    {
                        ListViewItem dirItem = listView2.Items.Add(dir.Name, 2);
                        dirItem.Name = dir.FullName;
                        dirItem.SubItems.Add("");
                        dirItem.SubItems.Add("文件夹");
                        dirItem.SubItems.Add(dir.LastWriteTimeUtc.ToString());
                    ...
                    foreach (FileInfo file in files)
                    {
                        ListViewItem fileItem = listView2.Items.Add(file.Name);
                        if (file.Extension == ".exe" || file.Extension == "")   //程序文件或无扩展名
                        {
                            Icon fileIcon = GetSystemIcon.GetIconByFileName(file.FullName);
                            imageList2.Images.Add(file.Name, fileIcon);
        ...
        private void Open2()
        {
            ...
            {
                //string newPath = listView2.SelectedItems[0].Name;
                string newPath = listView2.SelectedItems[0].Text;
                try
                {
                    //判断是目录还是文件
                    if (Directory.Exists(newPath))
        ...
        private void ListDrivers2()
        {
            ...
            foreach (DriveInfo driver in drivers)
            {
                TreeNode node = treeView2.Nodes.Add(driver.Name);
                ListViewItem item = listView2.Items.Add(driver.Name);
                item.Name = driver.Name;
                //判断驱动器类型，用不同图标显示
                switch (driver.DriveType)
                {
        ...
        private void toolStripButton8_Click_1(object sender, EventArgs e)
        {
            ...
            if (dir.Parent != null)
            {
                ListUpdate2(dir.Parent.FullName);
            }
            else
            {
                ListDrivers2();
